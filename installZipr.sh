...
installShared()
{
  ...
  then
    ...
    echo "BINDIR=\"\$(dirname \"\$(readlink -f \"\$0\")\")\"" >> "${DEST_DIR}${DEST_BIN}/${prg}"
    #export BINDIR="$(dirname "$(readlink -f "$0")")"
    echo "\"\${BINDIR}/../lib/p7zip/${prg}\" \"\$@\"" >> "${DEST_DIR}${DEST_BIN}/${prg}"
...
rm -f  "${DEST_DIR}${DEST_SHARE}/ZiprFM"
